...
from src.models.config import ModelDateWindow, load_model_pipeline_config
...
def test_chronological_split_construction_assigns_non_overlapping_windows() -> None:
    """Rows should be assigned into disjoint train, validation, and test windows."""
    ...
    assert dataset.groupby("split")["date"].nunique().to_dict() == {
        "train": 2,
        "validation": 1,
        "test": 1,
    }
    assert dataset.loc[dataset["split"] == "train", "date"].max() < dataset.loc[
        dataset["split"] == "validation", "date"
    ].min()
    assert dataset.loc[dataset["split"] == "validation", "date"].max() < dataset.loc[
        dataset["split"] == "test", "date"
    ].min()
...
def test_logistic_regression_baseline_output_shape() -> None:
    """Logistic baseline should emit auditable train and held-out prediction frames."""
    config = _build_config()
    bundle = build_modeling_dataset(
        feature_panel=_feature_panel_fixture(),
        monthly_panel=_monthly_panel_fixture(),
...
def test_random_forest_baseline_output_shape() -> None:
    """Random forest baseline should emit the same canonical prediction structure."""
    config = replace(_build_config(), execution=replace(_build_config().execution, selected_model="random_forest"))
    bundle = build_modeling_dataset(
        feature_panel=_feature_panel_fixture(),
        monthly_panel=_monthly_panel_fixture(),
        config=config,
...
def test_feature_importance_export_contains_all_features() -> None:
    ...
    assert artifacts.feature_importance["feature"].tolist() == sorted(
        config.dataset.feature_columns,
        key=lambda value: (
            -artifacts.feature_importance.set_index("feature").loc[value, "importance"],
            value,
        ),
    )
